...
from timm.models.helpers import build_model_with_cfg, pretrained_cfg_for_features
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, in_chans=3, num_classes=1000, global_pool='avg', drop_rate=0.0, head='classification'):
        ...
        print('Feature info', self.feature_info)
        self.out_channels = [
            in_chans
        ]
        for i in self.feature_info:
            self.out_channels.append(i['num_chs'])
...
def hrnet_cbam(pretrained=False, **kwargs):
    return _create_hrnet('hrnet_w64', pretrained, **kwargs)
